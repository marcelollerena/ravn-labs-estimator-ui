...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col bg-zinc-950">
      {/* Logo */}
      <div
        className={`flex h-12 shrink-0 items-center border-b border-zinc-800/50 ${collapsed ? 'justify-center px-2' : 'px-4'}`}
      >
        {collapsed ? (
          <span className="text-[13px] font-bold text-zinc-100">E</span>
        ) : (
          <span className="text-[13px] font-semibold tracking-tight text-zinc-100">Estimator</span>
        )}
      </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden font-['Inter',system-ui,sans-serif]">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {/* Content */}
        <main className="flex-1 overflow-y-auto bg-white dark:bg-zinc-900">{children}</main>
      </div>
    </div>
  );
}
