...
function StepIcon({ status }: { status: StepStatus }) {
  ...
  if (status === 'active') return <Loader2 size={18} className="animate-spin text-blue-500" />;
  if (status === 'error') return <XCircle size={18} className="text-red-500 dark:text-red-400" />;
    ...
    <div>
      ...
      <div className="mx-auto max-w-[520px] px-6 py-16">
        ...
        <div className="mb-8 flex items-center justify-center gap-3">
          ...
          <div>
            {fileName && (
              <p className="text-[13px] font-medium text-zinc-900 dark:text-zinc-100">{fileName}</p>
            )}
            {sourceFormat && (
